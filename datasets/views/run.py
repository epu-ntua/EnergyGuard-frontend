...
from ..services import provision_pilot_dataset, provision_user_datasets
...
def dataset_run(request, dataset_id):
    ...
    partner = dataset.pilot_partner
    if partner:
        # Pilot data lives in the data lake, not MinIO. We only tell the data
        # management server which partner to fetch; it does the download itself.
        try:
            provision_pilot_dataset(jupyterhub_username, partner, dataset_local_name)
        except requests.RequestException as exc:
            return JsonResponse(
                {"error": f"Failed to provision pilot dataset: {exc}"}, status=502
            )
    else:
        ...
        try:
            provision_user_datasets(
                jupyterhub_username, {minio_prefix: dataset_local_name}
            )
        except requests.RequestException as exc:
            return JsonResponse(
                {"error": f"Failed to provision dataset: {exc}"}, status=502
            )
